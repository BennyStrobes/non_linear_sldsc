...
def ldsc_mse_tf_loss_fxn(chi_sq, pred_tau, samp_size, ld_sq, middle_indices, snp_weights):
	# NEED TO: 
	...
	middle_indices_weights = tf.gather(snp_weights, middle_indices)
...
def ldsc_mse_tf_loss_fxn2(chi_sq, pred_tau, samp_size, ld_sq, middle_indices, snp_weights):
	# NEED TO: 
	### 1. GET RID OF NON-MIDDLE INDICES
	### 1. WEIGHT BY LD
	pred_chi_sq = (samp_size*tf.linalg.matmul(ld_sq, pred_tau)) + 1.0
...
def ldsc_mse_tf_loss_fxn3(chi_sq, pred_tau, samp_size, ld_sq_inv, middle_indices, snp_weights):
	# NEED TO: 
	### 1. GET RID OF NON-MIDDLE INDICES
	### 1. WEIGHT BY LD

	res = tf.linalg.matmul(ld_sq_inv, (chi_sq - 1.0)/samp_size)

	squared_error = tf.pow(res - pred_tau, 2)
...
def marginal_non_linear_sldsc_old(window_data, samp_size, model_type, temp_output_model_root, max_epochs=5):
	...
	for epoch_iter in range(max_epochs):
		...
		for window_iter in range(num_windows):
			...
			with tf.GradientTape() as tape:
				...
				loss_value = ldsc_mse_tf_loss_fxn(tf.convert_to_tensor(window_chi_sq.reshape(len(window_chi_sq),1), dtype=tf.float32), window_pred_tau, samp_size, tf.convert_to_tensor(squared_ld, dtype=tf.float32), window_middle_indices, middle_indices_weights.reshape(len(middle_indices_weights),1))
...
def marginal_non_linear_sldsc(window_data, samp_size, model_type, temp_output_model_root, max_epochs=5):
	...
	for epoch_iter in range(max_epochs):
		...
		for window_counter, window_iter in enumerate(np.random.permutation(range(num_windows))):
			...
			with tf.GradientTape() as tape:
				window_pred_tau = genomic_anno_to_gamma_model(window_genomic_anno, training=True)
				#loss_value = ldsc_tf_loss_fxn(window_chi_sq, window_pred_tau, samp_size, squared_ld, window_middle_indices, snp_weights)
				loss_value = ldsc_mse_tf_loss_fxn(window_chi_sq, window_pred_tau, samp_size, squared_ld, window_middle_indices, snp_weights)
				saved_norm_loss_value = loss_value
			grads = tape.gradient(loss_value, genomic_anno_to_gamma_model.trainable_weights)
			optimizer.apply_gradients(zip(grads, genomic_anno_to_gamma_model.trainable_weights))
			...
			# DEBUGGING
			# Initialize mapping from annotations to per snp heritability
			genomic_anno_to_gamma_model_norm = initialize_genomic_anno_model(model_type, annotation_data_dimension)
			optimizer = tf.keras.optimizers.Adam()
			loss_value_arr1 = []
			for itera in range(2000):
				print(itera)
				with tf.GradientTape() as tape:
					window_pred_tau = genomic_anno_to_gamma_model_norm(window_genomic_anno, training=True)
					loss_value = ldsc_mse_tf_loss_fxn2(window_chi_sq, window_pred_tau, samp_size, squared_ld, window_middle_indices, snp_weights)
				loss_value_arr1.append(loss_value)
				grads = tape.gradient(loss_value, genomic_anno_to_gamma_model_norm.trainable_weights)
				optimizer.apply_gradients(zip(grads, genomic_anno_to_gamma_model_norm.trainable_weights))

			genomic_anno_to_gamma_model_alt = initialize_genomic_anno_model(model_type, annotation_data_dimension)
			optimizer = tf.keras.optimizers.Adam()
			squared_ld_inv = tf.linalg.inv(squared_ld + (1e-3*np.eye(squared_ld.shape[1])))
			loss_value_arr2 = []
			for itera in range(2000):
				print(itera)
				with tf.GradientTape() as tape:
					window_pred_tau = genomic_anno_to_gamma_model_alt(window_genomic_anno, training=True)
					loss_value = ldsc_mse_tf_loss_fxn3(window_chi_sq, window_pred_tau, samp_size, squared_ld_inv, window_middle_indices, snp_weights)
				loss_value_arr2.append(loss_value)
				grads = tape.gradient(loss_value, genomic_anno_to_gamma_model_alt.trainable_weights)
				optimizer.apply_gradients(zip(grads, genomic_anno_to_gamma_model_alt.trainable_weights))
			pdb.set_trace()


			print('loss')
			print(saved_norm_loss_value)
...
# Model training
temp_output_model_root = output_dir + trait_name + '_nonlinear_sldsc_marginal_updates_mse_results_training_data_' + model_type + '_' + training_chromosome_type + '_annotations_to_gamma_model_temp'
...
training_data_tf_model_output_file = output_dir + trait_name + '_nonlinear_sldsc_marginal_updates_mse_results_training_data_' + model_type + '_' + training_chromosome_type + '_annotations_to_gamma_model'
