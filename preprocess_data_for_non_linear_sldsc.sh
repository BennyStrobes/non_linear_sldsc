...
trait_name="$4"
reference_genome_fasta_dir="$5"


if false; then
for chrom_num in {1..22}; do 
	sbatch preprocess_data_for_non_linear_sldsc_per_chrom.sh $ukbb_preprocessed_for_genome_wide_susie_dir $ldsc_baseline_ld_hg19_annotation_dir $preprocessed_data_for_non_linear_sldsc_dir $chrom_num $trait_name $reference_genome_fasta_dir
done
fi
